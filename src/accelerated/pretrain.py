...
class Trainer:
    def __init__(self, config: LaBraMConfig):
        ...
        self.accelerator = Accelerator(
            log_with="wandb", gradient_accumulation_steps=self.accum_steps
        )

        # Logging
        tracker_kwargs = {}
        self.use_wandb = config.logging.wandb_project is not None
        if self.use_wandb:
            log = config.logging
        ...
        self.model = setup_model()  # noqa: F821
        self.tokenizer = setup_tokenizer()  # noqa: F821
    ...
    def setup_model(self):
        ...
        """
        model_config = self.config.model  # noqa: F841
        ...
        if path is not None:
            cp = torch.load(
                path,
                map_location="cuda:0",  # Assume cuda:0 for training
    ...
    def setup_tokenizer(self):
        ...
        if tokenizer_name == "vqnsp_encoder_base_decoder_3x200x12":
            tokenizer = vqnsp_encoder_base_decoder_3x200x12(
                pretrained=True,
            ...
            tokenizer = vqnsp_encoder_large_decoder_3x200x24(
                pretrained=True,
                pretrained_weight=pretrained_weight,
                as_tokenzer=True,
                n_code=n_code,
    ...
    def train(self, loader):
        ...
        optimizer = getattr(torch.optim, self.config.train.opt)(
            self.model.parameters(), **self.config.train.opt_kwargs
        )
        scheduler = getattr(torch.optim.lr_scheduler, self.config.train.scheduler)(
            optimizer, **self.config.train.scheduler_kwargs
        )
