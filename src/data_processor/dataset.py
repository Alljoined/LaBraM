...
class SingleShockDataset(Dataset):
    ...
    def __init_dataset(self) -> None:
        ...
        for subject in self.__subjects:
            self.__global_idxes.append(
                global_idx
            )  # the start index of the subject's sample in the dataset
            subject_len = self.__file[subject]["eeg"].shape[1]
            # total number of samples
            total_sample_num = (
                subject_len - self.__window_size
            ) // self.__stride_size + 1
            # cut out part of samples
            start_idx = (
                int(total_sample_num * self.__start_percentage) * self.__stride_size
            )
            end_idx = (
                int(total_sample_num * self.__end_percentage - 1) * self.__stride_size
            )
    ...
    def __getitem__(self, idx: int):
        ...
        item_start_idx = (
            idx - self.__global_idxes[subject_idx]
        ) * self.__stride_size + self.__local_idxes[subject_idx]
        return self.__file[self.__subjects[subject_idx]]["eeg"][
    ...
    def __init_dataset(self) -> None:
        self.__datasets = [
            SingleShockDataset(
                ...
                self.__window_size,
                self.__stride_size,
                self.__start_percentage,
                self.__end_percentage,
            )
