...
def get_args():
    ...
    parser.add_argument(
        "--input_size", default=1600, type=int, help="EEG input size for backbone"
    )
    ...
    parser.add_argument(
        "--calculate_codebook_usage", action="store_true", default=False
    )
...
def main(args):
    ...
    datasets_train = [
        [
            "path/to/dataset1",
            "path/to/dataset2",
        ],  # e.g., 64 channels for dataset1 and dataset2
        [
            "path/to/dataset3",
            "path/to/dataset4",
        ],  # e.g., 32 channels for dataset3 and dataset4
    ]
    ...
    dataset_train_list, train_ch_names_list = utils.build_pretraining_dataset(
        datasets_train, time_window, stride_size=200
    )
    ...
    if args.calculate_codebook_usage:
        test_stats = calculate_codebook_usage(
            data_loader_val, model, device, log_writer, 0, args=args
        )
        exit(0)
