...
class NeuralTransformerForMaskedEEGModeling(nn.Module):
    def __init__(
        ...
        if use_shared_rel_pos_bias:
            self.rel_pos_bias = RelativePositionBias(
                window_size=(62, EEG_size // patch_size), num_heads=num_heads
            )
        else:
    ...
    def forward_features(
        self, x, input_chans, bool_masked_pos, output_hidden_states=False
    ):
    ...
    def forward(
        ...
        if bool_masked_pos is None:
            bool_masked_pos = torch.zeros(
                (x.shape[0], x.shape[1] * x.shape[2]), dtype=torch.bool
            ).to(x.device)
        if output_hidden_states:
            x, out = self.forward_features(
                x,
...
class NeuralTransformerForMEM(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.lm_head = nn.Linear(embed_dim, vocab_size)
        self.projection_head = nn.Sequential(nn.Linear(embed_dim, embed_dim), nn.ReLU())
